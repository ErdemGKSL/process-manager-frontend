...
<div class="bg-surface-800 w-full min-h-full sm:p-12 p-2 flex justify-center sm:items-center sm:pt-0 pt-16">
  <form class="variant-ghost-secondary sm:p-12 p-4 rounded-3xl w-[600px] max-w-full gap-2 sm:gap-8 flex flex-col font-mono font-extrabold sm:justify-start justify-around h-fit" on:submit={() => {
  ...
  <h1 class="text-3xl w-full text-center sm:text-3xl sm:text-left">Kayıt Ol</h1>
    <div class="flex flex-col gap-8">
      ...
      <div class="flex flex-row-reverse w-full justify-between items-center text-center">
        <button class="btn variant-ghost-primary w-fit" type="submit" disabled={!((username && password) && password === password2)}>Kayıt Ol</button>
